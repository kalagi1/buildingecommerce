    ...
    */
    ...
    public function setStatus( $projectId, Request $request ) {
        ...
        if ( $request->input( 'status' ) == 3 ) {
            ...
            $notificationText = 'Proje #' . $projectId . ' şu anda yayında! Tebrikler! Daha fazla detay için [Proje Detay Sayfası](' . route( 'project.detail', [ 'slug' => $project->slug,'id' => $project->id+2000000 ] ) . ').';
    ...
    public function setStatusGet( $projectId ) {
        ...
        DocumentNotification::create(
            [
                ...
                'item_id' => $project->id,
                'link' => route( 'project.detail', [ 'slug' => $project->slug,'id' => $project->id+2000000 ] ),
                'owner_id' => $project->user->id,
                'is_visible' => true,
            ]
        );
