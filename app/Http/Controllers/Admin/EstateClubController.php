...
class EstateClubController extends Controller
{
    ...
    public function changeStatus(Request $request, $userId, $action)
    {
        ...
        if ($action == 'approve') {
            ...
            // SMS metni oluştur
            $message = "Emlak Kulüp başvurunuz onaylanmıştır. Koleksiyon oluşturup kazanmaya başlayabilirsiniz.";
            ...
            // SMS metni oluştur
            $message = "Emlak Kulüp başvurunuz reddedilmiştir. Lütfen emlak sepette yönetimi ile iletişime geçiniz.";
    ...
    public function createCouponStore(Request $request, $userId)
    {
        ...
        if ($request->input('select_project_check') == 2) {
            for (
                $i = 0;
                $i < count($request->input('projects'));
                $i++
            ) {
                CouponItem::create([
        ...
        if ($request->input('select_housing_check') == 2) {
            for (
                $i = 0;
                $i < count($request->input('housings'));
                $i++
            ) {
                CouponItem::create([
