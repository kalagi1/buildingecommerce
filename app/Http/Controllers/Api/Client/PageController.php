    ...
    public function clientCollections()
{
    ...
    // Her öğe için kazanç miktarını hesapla ve verilere ekle
    foreach ($items as $item) {
        ...
        if ($item->item_type == 2) {
            ...
            $sales_rate_club = $sales_rate_club ?? ($rates->isNotEmpty() ? $rates->last()->sales_rate_club : null);

            $discountedPrice = 0; // Burada discountedPrice uygun bir şekilde elde edilmeli
            ...
            $discountedPrice = isset($discountRate) && $discountRate != 0 && isset($discountedPrice)
                ? $discountedPrice
                : (isset($item->project_values['price[]']) ? $item->project_values['price[]'] : $item->project_values['daily_rent[]']);
            $earningAmount = $discountedPrice * $sharePercent;
        }

        // Her öğenin kazanç miktarını formatlayarak ekleyelim
