...
class ReservationController extends Controller {
    ...
    ->first();
    $shareOpen = isset(
        json_decode( Housing::find( $request->input( 'housing_id' ) ?? 0 )->housing_type_data ?? '[]' )-> {
            'share-open1'}
        ) ? json_decode( Housing::find( $request->input( 'housing_id' ) ?? 0 )->housing_type_data ?? '[]' )-> {
            'share-open1'}
            : null;
            ...
            if ( $shareOpen && $lastClick ) {
                ...
                SharerPrice::create( [
                    ...
                    'status' => '0',
                    'balance' => ( $earnMoney * 5 ) / 100,
                    'earn' => ( $earnMoney * 5 ) / 100,
                    'earn2' => ( $earnMoney * 90 ) / 100,
                    'is_reservation' => 1,
