...
function FileUpload({
  ...
  const fileUpload = async (event) => {
    if (!document) {
      ...
      if (multiple) {
        ...
        var lastImageCount = projectData[fileName + "_imagesx"]
          ? projectData[fileName + "_imagesx"].length
          : 0;
        ...
        if (lastImageCount + files.length <= 40) {
          for (let i = 0; i < files.length; i++) {
            ...
            if (file.size > maxSize) {
              toast.error(
                `Dosya "${file.name}" çok büyük. Lütfen 3 MB'den küçük dosyalar yükleyin.`
              );
            ...
            if (requiredType) {
              ...
              if (!requiredType.includes(splitFile[splitFile.length - 1])) {
                toast.error(
                  "Dosya tipi " + requiredType.join(",") + " olmalıdır"
                );
                continue; // Büyük dosyayı yükleme, sonraki dosyaya geç
              }
            }
  ...
  const removeImage = (imageIndex) => {
    var newImages = projectData[fileName + "_imagesx"].filter(
      (image, imageOrder) => {
    ...
    <div class="project_imagex_flex">
      ...
      >
        ...
        <div
          ...
          id={fileName}
          className={
            "upload-container col-md-4 col-xl-3 cover-photo-area " +
            (allErrors.includes(fileName) ? "error-border" : "")
          }
        >
          <div className="border-container">
            ...
            <p>
              Bilgisayardan {document ? "Dosya" : "Fotoğraf"} Ekle{" "}
          ...
          projectData[fileName] ? (
            <div class="cover-document">
              <div class="has_file">
                ...
                <a
                  class="btn btn-info"
                  href="https://hisselitask.emlaksepette.com/housing_documents/document_temp106.pdf"
                  download=""
                >
        ...
        ) : projectData[fileName + "_imagex"] ||
          projectData[fileName + "_imagesx"] ||
          imageLoading ? (
          <div>
            {multiple ? (
              <span
                className="d-block badge badge-phoenix badge-phoenix-success inline-block"
                style={{ marginTop: "20px" }}
              >
                Yüklenen Resim Sayısı :{" "}
                {(projectData[fileName + "_imagesx"]
                  ? projectData[fileName + "_imagesx"].length
                  : 0) + (imageLoading ? uploadingOrder[0].length : 0)}{" "}
                / 40
              </span>
            ) : (
              ""
                ...
                <>
                  {projectData[fileName + "_imagesx"] ? (
                    <ReactSortable
                      ...
                      onSort={(e) => {
                        var newList = changeArrayValues(
                          projectData[fileName + "_imagesx"],
                          e.newIndex,
                          e.oldIndex
                        );
                        var newList2 = changeArrayValues(
                          projectData[fileName],
                          e.newIndex,
                          e.oldIndex
                        );
