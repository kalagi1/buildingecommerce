...
function ReactTable(props) {
  ...
  useEffect(() => {
    if(!loading){
      ...
      axios
        .get(
          "http://private.emlaksepette.com/react/my_projects?status=" +
            tabIndex +
            "&start=0&take=" +
            rowPerPage
        )
  ...
  const handleChangePage = (event, newPage) => {
    ...
    axios
      .get(
        `http://private.emlaksepette.com/react/my_projects?status=${tabIndex}&start=${start}&take=${rowPerPage}`
      )
      .then((res) => {
        setRows(res.data.data);
        setTotalProjectsCount(res.data.total_projects_count);
  ...
  const handleChangeRowsPerPage = (event) => {
    ...
    axios
      .get(
        `http://private.emlaksepette.com/react/my_projects?status=${tabIndex}&start=0&take=${event.target.value}`
      )
      .then((res) => {
        setRows(res.data.data);
        setTotalProjectsCount(res.data.total_projects_count);
    ...
    <div className="estate-table">
                ...
                <TableBody>
                  {rows.map((row) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <a
                          href={`http://private.emlaksepette.com/hesabim/projects/${row.id}/housings_v2`}
                      ...
                      <TableCell>
                        <div className="d-flex">
                          <a
                            ...
                            href={`http://private.emlaksepette.com/hesabim/projects/${row.id}/logs`}
                          >
                            İşlem Kayıtları
                          </a>
                          <a
                            class="badge badge-phoenix badge-phoenix-success mx-3"
                            href={`http://private.emlaksepette.com/hesabim/edit_project_v2/${row.slug}/${row.id}`}
                          >
                            Genel Düzenleme
                          </a>
                        </div>
      ...
      >
        <ul className="popover-project-actions">
          <li>
            <a href={`http://private.emlaksepette.com/hesabim/projects/${selectedRow?.id}/housings_v2`}>İlanları Düzenle</a>
          </li>
          <li><a href={`http://private.emlaksepette.com/hesabim/projects/${selectedRow?.id}/logs`}>İşlem Kayıtları</a></li>
          <li><a href={`http://private.emlaksepette.com/hesabim/edit_project_v2/${selectedRow?.slug}/${selectedRow?.id}`}>Genel Düzenleme</a></li>
          <li>
