...
function ReactTable(props) {
  ...
  useEffect(() => {
    if(!loading){
      ...
      axios
        .get(
          "http://buildingecommerce.test/react/my_projects?status=" +
            tabIndex +
            "&start=0&take=" +
            rowPerPage
        )
  ...
  const handleChangePage = (event, newPage) => {
    ...
    axios
      .get(
        `http://buildingecommerce.test/react/my_projects?status=${tabIndex}&start=${start}&take=${rowPerPage}`
      )
      .then((res) => {
        setRows(res.data.data);
        setTotalProjectsCount(res.data.total_projects_count);
  ...
  const handleChangeRowsPerPage = (event) => {
    ...
    axios
      .get(
        `http://buildingecommerce.test/react/my_projects?status=${tabIndex}&start=0&take=${event.target.value}`
      )
      .then((res) => {
        setRows(res.data.data);
        setTotalProjectsCount(res.data.total_projects_count);
  ...
  return (
    <div className="estate-table">
      ...
      <div className="project-table-menu">
        <ul>
          <li style={{width : '5%'}}>No</li>
          <li style={{width : '12%'}}>Proje Görseli</li>
          <li style={{width : '23%',alignItems:'flex-start'}}>Proje Adı</li>
          <li style={{width : '7%'}}>İlan Sayısı</li>
          <li style={{width : '7%'}}>Satış Adeti</li>
          <li style={{width : '12%'}}>Onaydaki Siparişler</li>
          <li style={{width : '8%'}}>Satışa Kapalı</li>
          <li style={{width : '8%'}}>Satışa Açık</li>
          <li style={{width : '9%'}}>Yayın Durumu</li>
          <li style={{width : '5%'}}>İşlemler</li>
        </ul>
      </div>
      ...
      >
        <ul className="popover-project-actions">
          <li>
            <a href={`http://buildingecommerce.test/hesabim/projects/${selectedRow?.id}/housings_v2`}>İlanları Düzenle</a>
          </li>
          <li><a href={`http://buildingecommerce.test/hesabim/projects/${selectedRow?.id}/logs`}>İşlem Kayıtları</a></li>
          <li><a href={`http://buildingecommerce.test/hesabim/edit_project_v2/${selectedRow?.slug}/${selectedRow?.id}`}>Genel Düzenleme</a></li>
          <li>
