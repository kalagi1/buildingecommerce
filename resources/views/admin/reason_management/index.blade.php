            ...
            <div class="mb-9">
                <div id="emailTemplateList"
                    ...
                    <div class="row justify-content-between mb-4 gx-6 gy-3 align-items-center">
                        <div class="col-auto">
                            <h3 class="mb-0">Sebep Şablonları<span
                                    class="fw-normal text-700 ms-3">({{ count($templates) }})</span>
                                </h2>
                        </div>
                        <div class="col-auto">
                            <div class="col-auto"><a class="btn btn-primary px-5"
                                    href="{{ route('admin.reason.templates.create') }}"><i
                                        class="fa-solid fa-plus me-2"></i>Yeni
                                    Ekle</a></div>
                        </div>
                    </div>
                    @if (session('success'))
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th class="sort white-space-nowrap align-middle ps-0" scope="col"
                                            data-sort="name">Şablon Adı</th>
                                        <th class="sort white-space-nowrap align-middle ps-0" scope="col"
                                            data-sort="subject">Konu</th>
                                        <th>İşlemler</th>
                                    </tr>
                                </thead>
                                <tbody class="list" id="email-template-list-table-body">
                                    @foreach ($templates as $template)
                                        <tr class="position-static">
                                            ...
                                            <td>
                                                <div id="contentDiv_{{ $template->id }}"></div>
                                                <script>
                                                    var content_{{ $template->id }} = `{!! addslashes($template->content) !!}`; // Tek tırnakları kaçır
                                                    document.getElementById("contentDiv_{{ $template->id }}").innerHTML = content_{{ $template->id }};
                                                </script>
                                            </td>
                                            <td>
                                                <a href="{{ route('admin.reason.templates.edit', ['defaultMessage' => $template]) }}"
                                                    class="btn btn-sm btn-primary">Düzenle</a>

                                                <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal"
                                                    data-bs-target="#deleteModal{{ $template->id }}">Sil</button>
                                                        ...
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h5 class="modal-title"
                                                                    id="deleteModalLabel{{ $template->id }}">Sil</h5>
                                                            ...
                                                            <div class="modal-footer">
                                                                <form
                                                                    action="{{ route('admin.reason.templates.destroy', ['defaultMessage' => $template]) }}"
