            ...
            <table class="table table-sm fs--1 mb-0">
                <thead>
                    <tr>
                        ...
                        <th class="sort white-space-nowrap align-middle pe-3" scope="col"
                            data-sort="pay_type" style="width: 18%">Bilgileri Önizle</th>
                        <th class="sort white-space-nowrap align-middle pe-3" scope="col"
                            data-sort="order_status">Onay/Red</th>
                        <th class="sort white-space-nowrap align-middle pe-3" scope="col"
                    ...
                    @if ($expectedCall->count() > 0)
                        @foreach ($expectedCall as $item)
                            ...
                            <tr  class="table">
                                ...
                                <td class="order_amount align-middle  fw-semibold text-body-highlight">
                                    <button type="button" class="badge badge-phoenix fs--2 badge-phoenix-info" data-bs-toggle="modal" data-bs-target="#onizlemeModal{{$item->id}}">
                                        Bilgileri Önizle
                                      </button>
                                </td>
                                ...
                                <td class="order_amount align-middle fw-semibold text-body-highlight">
                                    @if($item->status == 0)
                                        <a href="{{ route('admin.institutional.give.approval', ['user_id' => $item->user_id]) }} "
                                             class="badge badge-phoenix fs--7 badge-phoenix-success" style="padding: 5px 45px 5px 45px;">
                                             Onayla</a><br>
                                        <a href="{{ route('admin.institutional.reject', ['user_id' => $item->user_id]) }} "
                                             class="badge badge-phoenix fs--4 badge-phoenix-danger" style="padding: 5px 45px 5px 45px;margin-top:5px;">Reddet</a><br>
                                    @else
                                        -    
                                    @endif
                                </td>    
                                ...
                                <!--önizleme Modal -->
                                <div class="modal fade" id="onizlemeModal{{$item->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog modal-lg">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="exampleModalLabel">{{$item->id}} numaralı önizleme</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <div class="row">
                                                    <div class="col-md-6">
                                                        <p><strong>Yetkili İsim Soyisim:</strong> {{$item->username}}</p>
                                                        ...
                                                        <p><strong>Kurumsal Hesap Türü:</strong> {{$item->corporate_account_type}}</p>
                                                        <p><strong>İl:</strong> {{$item->city->title}}</p> <!-- İl title değeri -->
                                                        <p><strong>İlçe:</strong> {{$item->county->title}}</p> <!-- İlçe title değeri -->
                                                    </div>
                                                    <div class="col-md-6">
