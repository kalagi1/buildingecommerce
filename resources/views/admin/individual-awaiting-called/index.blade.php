...
@section('content')
    <div class="content">
        <h4 class="tableHeader" style="">Bireysel Onay Süreci</h4>
                ...
                <table class="table table-sm fs--1 mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="sort white-space-nowrap align-middle pe-3" scope="col" data-sort="order_date">
                                E-Mail</th>
                            <th class="sort white-space-nowrap align-middle pe-3" scope="col" data-sort="order_date">Cep
                                Telefonu</th>
{{-- 
                            <th class="sort white-space-nowrap align-middle pe-3" scope="col" data-sort="order_date">
                                Sabit Telefonu</th> --}}
                            <th class="sort white-space-nowrap align-middle pe-3" scope="col" data-sort="order_date">
                                Email Doğrulaması</th>
                            <th class="sort white-space-nowrap align-middle pe-3" scope="col" data-sort="customer">Cep
                                Telefonu Doğrulaması</th>
                            {{-- <th class="sort white-space-nowrap align-middle pe-3" scope="col" data-sort="sales_person">
                                Belge Doğrulaması</th> --}}
                            <th class="sort white-space-nowrap align-middle pe-3" scope="col" data-sort="order_amount"
                                style="width: 18%">Arandı Mı ?</th>
                                ...
                                <tr class="table">
                                    ...
                                    {{-- <td class="order_no align-middle  fw-semibold text-body-highlight">
                                        ...
                                        @if ($item->email_verified_at && $item->phone_verification_status == 1 && $item->is_show_files == 1)
                                            <a href="{{ route('admin.user.show-corporate-account', ['user' => $item->id]) }}"
                                                class="badge badge-phoenix fs--2 badge-phoenix-success ">Belgeleri
                                                Görüntüle</a>
                                        @endif
                                    </td> --}}
                                    ...
                                    <td class="order_no align-middle  fw-semibold text-body-highlight">
                                        @if ($item->mobile_phone)
                                            <a href="tel:{{ $item->mobile_phone }}"
                                                class="badge badge-phoenix fs--2 badge-phoenix-info">{{ $item->mobile_phone }}</a>
                                        @else
                                            -
                                        @endif
                                    </td>
                                    ...
                                    <td class="order_date align-middle white-space-nowrap text-body-tertiary  text-wrap">
                                        @if ($item->email_verified_at == null)
                                            <a href="{{ route('admin.mail.verification', ['id' => $item->id]) }}"
                                                class="badge badge-phoenix fs--2 badge-phoenix-warning">Doğrulama Linki
                                                Gönder</a>
                                        @else
                                            <a href="#"
                                                class="badge badge-phoenix fs--2 badge-phoenix-success">Doğrulanmış</a>
                                        @endif
                                    </td>
                                    ...
                                    <td class="ad_no align-middle  fw-semibold text-body-highlight">
                                        @if ($item->phone_verification_status == 0)
                                            <a href="#"
                                                class="badge badge-phoenix fs--2 badge-phoenix-warning">Doğrulanmamış</a>
                                        @else
                                            <a href="#"
                                                class="badge badge-phoenix fs--2 badge-phoenix-success">Doğrulanmış</a>
                                        @endif
                                        ...
                                        @if ($item->email_verified_at && $item->phone_verification_status == 1)
                                            @if ($item->corporate_account_status == 0)
                                                @if ($item->is_show_files == 0)
                                                    <a href="{{ route('admin.document.load.page', ['id' => $item->id]) }}"
                                                        class="badge badge-phoenix fs--2 badge-phoenix-info belgeYuklemeEkrani">Belge
                                                        Yükleme Ekranı Göster</a>
                                                @else
                                                    <a href="#"
                                                        class="badge badge-phoenix fs--2 badge-phoenix-success ">Belge
                                                        Yükleme Ekranına Yönlendirildi</a><br>
                                                   
                                                @endif
                                            @endif
                                        @else
                                            @if ($item->email_verified_at && $item->phone_verification_status == 0)
                                                <span class="badge badge-phoenix fs--2 badge-phoenix-danger">Telefon
                                                    Doğrulaması yapılmadı</span>
                                            @endif
                                            @if ($item->email_verified_at == null && $item->phone_verification_status == 0)
                                                <span class="badge badge-phoenix fs--2 badge-phoenix-danger">Email ve
                                                    Telefon Doğrulaması yapılmadı</span>
                                            @endif
                                    ...
                                    <td class="order_amount align-middle  fw-semibold text-body-highlight">
                                        @if ($item->is_called == 1)
                                            <a href="#"
                                                class="badge badge-phoenix fs--2 badge-phoenix-success">Arandı</a>
                                        @else
                                            <a href="{{ route('admin.searched', ['id' => $item->id]) }}"
                                                class="badge badge-phoenix fs--2 badge-phoenix-warning isCalled">Aranacak</a>
                                        @endif
                                    </td>
            ...
            $('.belgeYuklemeEkrani').click(function(e) {
                ...
                swal({
                    ...
                    if (willMarkAsSearched) {
                        ...
                        swal("Başarılı!",
                            "Kullanıcı belge yükleme alanına başarıyla yönlendirildi.",
                            "success");
                    } else {
                    ...
                    .then((willMarkAsSearched) => {
                        if (willMarkAsSearched) {
                            window.location.href =
                                url; // SweetAlert onaylandıktan sonra doğrudan yönlendirme yap
                        } else {
                            swal("İşlem iptal edildi.", {
                                icon: "error",
                            });
