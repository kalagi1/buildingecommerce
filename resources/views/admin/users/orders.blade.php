                                    ...
                                    <div class="project-inner project-head">
                                        <div class="homes">
                                            ...
                                            <span class="text-center d-block w-100 mt-3"
                                                style="font-size:9px !important;border:none !important">{{ $brand->name }}
                                            </span>
                                            <div class="form-check form-switch text-center d-block w-100 mt-3"
                                                style="    margin: 0 auto;
                                            display: flex !important;align-items:center;justify-content:center">
                                                <input class="form-check-input" type="checkbox"
                                                    id="brandToggle_{{ $brand->id }}"
                                                    {{ $brand->is_show == 'yes' ? 'checked' : '' }}
...
@section('scripts')
    ...
    <script>
                ...
                onUpdate: function(evt) {
                    ...
                    const orders = Array.from(item.parentNode.getElementsByClassName('brand-item'))
                        .map(function(element) {
                            return {
                                id: element.dataset
                                    .id, // Add the actual identifier for your brand item
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    // Update the data-is-show attribute
