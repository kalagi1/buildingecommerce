                ...
                <div class="row justify-content-between mb-4 gx-6 gy-3 align-items-center">
                    ...
                    <div class="col-auto">
                        <div class="col-auto"><a class="btn btn-primary px-5"
                                href="{{ route('admin.bank_account.create') }}"><i class="fa-solid fa-plus me-2"></i>Yeni
                                Ekle</a></div>
                    </div>
                </div>
                @if (session('success'))
                ...
                <div class="card shadow-none border border-300 my-4 p-5">
                    <div class="table-responsive scrollbar">
                        <table class="table fs--1 mb-0 border-top border-200">
                            <thead>
                                <tr>
                                    <th style="width:10%;">Banka Resmi</th>
                                    <th class="sort white-space-nowrap align-middle ps-0" scope="col"
                                        data-sort="name">Alıcı Adı</th>
                                    <th class="sort white-space-nowrap align-middle ps-0" scope="col"
                                        data-sort="subject">Iban</th>
                                    <th>İşlemler</th>
                                </tr>
                            </thead>
                            <tbody class="list" id="email-template-list-table-body">
                                @foreach ($bankAccounts as $bankAccount)
                                    <tr class="position-static">
                                        <td><img style="max-height: 50px;" src="{{ URL::to('/').'/'.$bankAccount->image }}" alt=""></td>
                                        <td>{{ $bankAccount->receipent_full_name }}</td>
                                        <td>{{ $bankAccount->iban }}</td>
                                        <td>
                                            <a href="{{ route('admin.bank_account.edit', $bankAccount->id) }}"
                                                class="btn btn-sm btn-primary">Düzenle</a>
                                            <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal"
                                                data-bs-target="#deleteModal{{ $bankAccount->id }}">Sil</button>

                                            <!-- Silme işlemi için modal -->
                                            <div class="modal fade" id="deleteModal{{ $bankAccount->id }}" tabindex="-1"
                                                aria-labelledby="deleteModalLabel{{ $bankAccount->id }}"
                                                aria-hidden="true">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title"
                                                                id="deleteModalLabel{{ $bankAccount->id }}">Sil</h5>
                                                        ...
                                                        <div class="modal-footer">
                                                            <form
                                                                action="{{ route('admin.bank_account.destroy', $bankAccount->id) }}"
