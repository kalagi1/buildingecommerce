  ...
  @section('scripts')
    ...
    <script>
        ...
        projects.forEach(function(project) {
            ...
            activeCell.innerHTML = project.status == 1 ? '<span class="badge badge-success">Aktif</span>' : project.status == 2 ? '<span class="badge badge-danger">Admin Onayı Bekliyor</span>' : project.status == 3 ? '<span class="badge badge-danger">Admin Tarafından Reddedildi</span>' : project.status ==  6 ? '<span class="badge badge-danger"><i class="fa fa-clock"></i> Süresi Bitti</span><span class="badge badge-phoenix badge-phoenix-primary c-pointer extend-time" style="margin-left:5px;" project_id="'+project.id+'"><i class="fa fa-plus"></i> Süresini Uzat</span>' : project.status ==  7 ? '<span class="badge badge-phoenix badge-phoenix-warning"><i class="fa fa-clock"></i> Ödeme onayı bekliyor</span>' : '<span class="badge badge-danger">Pasif</span>';
            ...
            success: function (response) {
              ...
              var html = '';
                html = '<tr>'+
                  '<td>'+
                    '<label for="">Mevcut Bitiş Tarihi</label>'+
                    '<p>'+(months[currentEndDate.getMonth()])+' '+currentEndDate.getDay()+', '+currentEndDate.getFullYear()+'</p>'+
                  '</td>'+
                  '<td>'+
                    '<label for="">Yeni Bitiş Tarihi</label>'+
                    '<p>'+(months[newEndDate.getMonth()])+' '+newEndDate.getDay()+', '+newEndDate.getFullYear()+'</p>'+
                  '</td>'+
                  '<td>'+
                    '<label for="">Fiyat</label>'+
        ...
        $('.bank-account').click(function(){
          ...
          $.ajax({
            ...
            success: function (response) {
              ...
              $('.bank_description_text').html(selectedProjectName+' projesinin süresini '+selectedMonthValue+' ay uzatma')
            },
            error: function (xhr) {
                // Hata durumunda yapılacak işlemler burada
                swal('Hata!', 'Proje silinirken bir hata oluştu.', 'error');
