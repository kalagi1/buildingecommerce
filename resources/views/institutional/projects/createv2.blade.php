            ...
            <div class="second-area @if ($tempDataFull->step_order != 2) d-none @endif">
                <div class="row">
                    ...
                    <div class="form-area mt-4">
                        ...
                        <h4 class="mb-3">Kaç Adet Konutunuz Var</h4><input
                            value="{{ isset($tempData->house_count) ? $tempData->house_count : '' }}"
                            onkeyup="changeData(this.value,'house_count')" class="form-control mb-5" type="text"
                            ...
                            <div class="card">
                                <div class="row px-5 py-4">
                                    <div class="col-md-12 statue-text">
                                        <label for="">Proje Konutlarının Statüsü Nedir? <span
                                        ...
                                        <select multiple name="housing_status" id="housing_status" aria-label="category">
                                            ...
                                            @foreach ($housing_status as $status)
                                                <option @if (isset($tempData->statuses) && in_array($status->id, $tempData->statuses)) selected @endif
                            ...
                            <div class="row single-price-project-area @if (
                                ...
                                <div>
                                    <label for="" class="c-pointer redirect-back-pricing"><i
                                            class="fa fa-chevron-left"></i> Seçime geri dön</label>
                                </div>
                                        ...
                                        <div class="pricing-item" onclick="changeData({{ $price->id }},'price_id')">
                                            <div class="pricing-item-inner">
                                                <div style="margin-right: 20px">
                                                    <input type="radio" style="display: none;margin:0 auto;">
                                                    <div
                                                        class="price-radio @if (isset($tempData->price_id) && $price->id == $tempData->price_id) select @endif">
                                                        <div class="inside"></div>
                                                    </div>
                                                </div>
                                                <div>
                    ...
                    <div class="col-md-5">
                        <div class="finish-area">
                            ...
                            <div class="text"><a href="{{ route('institutional.projects.index') }}"
                                    class="btn btn-info">Projelerime yönlen</a></div>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        success: function(response) {
                            ...
                            $('.next_house').click(function() {
                                ...
                                if (nextHousing) {
                                    ...
                                    $('.item-left-area').eq(indexItem + 1).find('p').addClass(
                                        'active');
                                } else {
                                    $('html, body').animate({
                                        scrollTop: $('.tab-pane.active').offset().top -
                                ...
                                for (var lm = 0; lm < json.length; lm++) {
                                    if (json[lm].type == "checkbox-group") {
                                        for (var i = 0; i < json[lm].values.length; i++) {
                                            var isChecked = $('input[name="' + (json[lm].name
                                                    .replace('[]', '')) + (order + 1) +
                                                '[][]"][value="' + json[lm].values[i]
                                                .value + '"]' + '').is(':checked')
                                            if (isChecked) {
                                                ...
                                                formData.append('value', json[lm].values[i]
                                                    .value);
                                        ...
                                        $('.tab-pane.active').find(
                                                '.cover-image-by-housing-type').parent('div')
                                            .find('.project_imaget').remove();
                                        $('.tab-pane.active').find(
                                                '.cover-image-by-housing-type').parent('div')
                                            .append(cloneImage)
                            ...
                            $('.rendered-form select').change(function() {
                                ...
                                formData.append('_token', csrfToken);
                                formData.append('value', $(this).val());
                                formData.append('order', parseInt($(this).closest('.tab-pane')
                                    .attr('id').replace('TabContent', "")) - 1);
                                formData.append('key', $(this).attr('name').replace("[]", ""));
                                formData.append('item_type', 1);
                                $.ajax({
                                    type: "POST",
                    ...
                    success: function(response) {
                        ...
                        $('.next_house').click(function() {
                            ...
                            if (nextHousing) {
                                ...
                                $('.tab-pane').eq(indexItem + 1).addClass('active');
                                $('.item-left-area p').removeClass('active');
                                $('.item-left-area').eq(indexItem + 1).find('p')
                                    .addClass('active');
                            } else {
                                $('html, body').animate({
                                    scrollTop: $('.tab-pane.active').offset()
                        ...
                        $('.cover-image-by-housing-type').change(function() {
                            ...
                            if (input.files && input.files[0]) {
                                ...
                                reader.onload = function(e) {
                                    ...
                                    $('.tab-pane.active .cover-image-by-housing-type')
                                        .parent('div').find('.project_imaget')
                                        .remove()
                                    $('.tab-pane.active .cover-image-by-housing-type')
                        ...
                        $('#tablist').attr('style', 'height:' + $('.tab-content').css(
                            'height') + ' !important;flex-wrap:nowrap;overflow:scroll;')
                    },
                    error: function(error) {
                        console.log(error)
                    }
    ...
    <script>
        ...
        $('.finish-button').click(function(e) {
            ...
            if ($('.pricing-item-first .price-radio.select').length == 0) {
                next = false;
                if (topError) {
                    if ($('.pricing-select-first').offset().top - parseFloat($('.navbar-top').css('height')) - 100 <
                        topError) {
                        topError = $('.pricing-select-first').offset().top - parseFloat($('.navbar-top').css(
                            'height')) - 100;
                    }
                } else {
                    topError = $('.pricing-select-first').offset().top - parseFloat($('.navbar-top').css(
