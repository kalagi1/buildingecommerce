                            ...
                            <div class="max-character-input">
                                <div class="row" style="align-items:center;">
                                    <div class="input col-md-10">
                                        <input type="text" value="{{isset($tempData->project_title) ? $tempData->project_title : ''}}" name="name" class="form-control advert_title">
                                    </div>
                                    <div class="col-md-2">
                                        <label class="max-character" for="">{{isset($tempData->project_title) ? Str::length(trim($tempData->project_title)) : '0'}}/70</label>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="cover-photo">
                                @if(isset($tempData->image) &&$tempData->image)
                                    <div class="project_imagex">
                                        <img src="{{URL::to('/').'/'.str_replace("public/", "storage/", $tempData->image)}}" alt="">
                                    </div>
                                @endif
                            </div>
                        </div>
            ...
            $.ajax({
                ...
                success: function(response) {
                        ...
                        for(var j = 0 ; j < formInputs.length; j++){
                            if(formInputs[j].type == "number" || formInputs[j].type == "text"){
                                ...
                                if(getOldData(i,inputName) != undefined){
                                    $($('input[name="'+formInputs[j].name+'"]')[i-1]).val(getOldData(i,inputName));
                                }
                                        ...
                                        JSON.parse(getOldData(i,inputName)).map((checkbox) => {
                                            ...
                                            if($(item).attr("value").trim() == "taksitli"){
                                                if($(item).attr("value") != undefined && checkbox.trim() == $(item).attr("value").trim()){
                                                    $(item).closest('.tab-pane').find('second-payment-plan').closest('div').removeClass('d-none')
                                                    $(item).attr('checked','checked')
                                                }else{
                                                    $(item).closest('.tab-pane').find('second-payment-plan').closest('div').addClass('d-none')
                                                }
                                            }else{ 
                                                if($(item).attr("value") != undefined && checkbox.trim() == $(item).attr("value").trim()){
                                                    $(item).attr('checked','checked')
                                ...
                                inputNamex = inputNamex.split('[]')
                                if(getOldData(i,inputName) != undefined){
                                    $($('input[name="'+formInputs[j].name+'"]')[i-1]).parent('div').append('<div class="project_imaget"><img src="{{URL::to("/")}}/project_housing_images/'+getOldData(i,inputName)+'"></div>');
                                }
                            }
                    ...
                    $('.copy-item').change(function(){
                            ...
                            for(var lm = 0 ; lm < json.length; lm++){
                                if(json[lm].type == "checkbox-group"){
                                    arrayValues[json[lm].name.replace("[]", "").replace("[]", "")+(currentOrder + 1)] = [];
                                    for(var i = 0 ; i < json[lm].values.length; i++){
                                        ...
                                        if(isChecked){
                                            ...
                                            arrayValues[json[lm].name.replace("[]", "").replace("[]", "")+(currentOrder + 1)].push(json[lm].values[i].value)
                                        }else{
                                            transactionIndex++;
                    ...
                    $('.cover-image-by-housing-type').change(function(){
                        ...
                        if (input.files && input.files[0]) {
                            ...
                            reader.onload = function(e) {
                                ...
                                $('.tab-pane.active .cover-image-by-housing-type').closest('.formbuilder-file').find('.project_imaget').eq(0).remove()
            ...
            if (input.files && input.files[0]) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        reader.onload = function(e) {
                        // Resmi görüntülemek için bir div oluşturun
                        var imageDiv = $('<div class="project_imagex" order="storage/project_images/'+response+'"></div>');

                        // Resmi oluşturun ve div içine ekleyin
                        var image = $('<img>').attr('src', '{{URL::to('/')}}/storage/project_images/'+response);
            ...
            if(next){
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(response.status){
                            window.location.href = "{{route('institutional.projects.index')}}?update_item=1"
                        }
                    }
                })
            }else{
