      ...
      <div class="row g-5">
        <div class="col-12 col-xl-8">
          ...
          <input type="file" name="project_images[]" multiple class="mb-4" id="">
            ...
            <h4 class="mb-3">Kaç Adet Konutunuz Var</h4><input class="form-control mb-5" type="text" id="house_count" name="house_count" value="{{$project->room_count}}" placeholder="Kaç Adet Konutunuz Var" />
    ...
    <script>
        jQuery($ => {
          Dropzone.autoDiscover = false;
            var myDropzone = new Dropzone("#cover_photo", {
                url: "yukleme_sonucu.php", // Sunucuya gönderilecek dosyaların işleneceği URL
                paramName: "images", // Sunucuya gönderilen dosyaların parametre adı
                maxFilesize: 5, // Maksimum dosya boyutu (MB)
                acceptedFiles: "image/*", // Kabul edilen dosya türleri (sadece resim dosyaları)
                addRemoveLinks: true, // Yüklü dosyaları kaldırma bağlantıları göster
                dictDefaultMessage: "Dosyaları buraya sürükleyin veya tıklayın", // Varsayılan mesaj
                autoProcessQueue: false, // Dosyaları otomatik olarak yükleme
            });
            ...
            $.ajax({
              ...
              method: 'GET',
              data: { city: {{$project->city_id}} }, // Şehir verisini isteğe ekle
              ...
              success: function(response) {
                  ...
                  for (var i = 0; i < response.length; i++) {
                      countiesSelect.append($('<option>', {
                          ...
                          selected : response[i].id == {{$project->county_id}}
                      }));
                  }
              },
                    ...
                    for(var j = 2 ; j < formInputs.length; j++){
                      if(formInputs[j].type == "number" || formInputs[j].type == "text"){
                        ...
                        $($('input[name="'+checkboxName+[i]+'[][]"]')).map((key,item) => {
                          console.log(JSON.parse(oldData[inputNamex[0]][i - 1]))
