...
@if ($offSaleValue == '[]' && $priceValue)
    @if ($sold && ($sold->status != '1' && $sold->status != '0'))
        @if ($offer && in_array($i + 1, json_decode($offer->project_housings)))
            <h6 style="color: #D32729 !important;position: relative;top:4px;font-weight:600;font-size: 11px;text-decoration:line-through;margin-right:5px">
        ...
        @else
            <h6 style="color: #D32729 !important;position: relative;top:4px;font-weight:600">
                {{ number_format($priceValue, 0, ',', '.') }} ₺
            </h6>
        @endif
    @else
