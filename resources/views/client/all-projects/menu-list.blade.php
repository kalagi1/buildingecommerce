...
@section('scripts')
    ...
    for (var i = dynamicIndex; i < pathnameParts.length; i++) {
        if (pathnameParts[i] !== 'kategori' && pathnameParts[i] !== '') {
            ...
            if (cityCountyNeighborhoodCount > 3) { // Eğer il, ilçe ve mahalle üç kez geldiyse, daha fazlasını engelle
                break;
            }
        }
    }
    ...
    var indexToAddSlug = dynamicIndex; // Default index
    if (type === 'city') {
        indexToAddSlug++;
    } else if (type === 'county') {
        indexToAddSlug = dynamicIndex + 1;
    } else if (type === 'neighborhood') {
        indexToAddSlug = dynamicIndex + 2;
    }
