    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function buildNewUrl(slug, type) {
                ...
                var indexToSlice = 3; // Default slice index, after '/kategori/emlak-ilanlari'

                // Determine the index to slice based on the number of existing parts
                if (pathnameParts.length > 3) {
                    indexToSlice = pathnameParts.length;
                }

                // Ensure that only one type of slug is added at a time, based on the type (city, county, neighborhood)
                if (type === 'city') {
                    indexToSlice = 4; // Slice after city slug
                } else if (type === 'county') {
                    indexToSlice = 5; // Slice after county slug
                } else if (type === 'neighborhood') {
                    indexToSlice = 6; // Slice after neighborhood slug
                }

                // Remove any existing slugs beyond the determined index
