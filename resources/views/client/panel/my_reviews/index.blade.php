...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-5">
        ...
        <div class="text-header p-0" id="yeniMusteriler" style="border-radius:7px;">
                    ...
                    <div class="project-table-content user-item">
                        <ul style="gap: 20px">
                            ...
                            <li style="width: 10%; align-items: flex-start;">
                                @if ($comment->type == 'Emlak')
                                    <a
                                        href="{{ route('project.housing.detail', ['slug' => $comment->housing->slug]) }}">{{ $comment->housing->id + 2000000 }}</a>
                                @else
                                    <a
                                        href="{{ route('project.housing.detail', ['slug' => $comment->project->slug]) }}">{{ $comment->project->id + 1000000 }}</a>
                                @endif
                            </li>
                                ...
                                @if ($comment->rate)
                                    @for ($i = 0; $i < $comment->rate; $i++)
                                        <svg viewBox="0 0 14 14" class="widget-svg"
                                            style="width: 10.89px; height: 10.89px;">
                                            <path class="star"
            ...
            <div id="pagination-controls" class="d-flex"
                style="margin-top: 20px; justify-content: space-between !important;">
